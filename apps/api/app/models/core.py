...
class ExecutiveBrief(Base):
    ...
    feedbacks = relationship(
        "BriefFeedback", back_populates="brief", cascade="all, delete-orphan"
    )
...
class BriefFeedback(Base):
    ...
    brief_id = Column(
        UUID(as_uuid=True),
        ForeignKey("executive_briefs.id", ondelete="CASCADE"),
        nullable=False,
    )
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
...
class KnowledgeReview(Base):
    ...
    )  # 'fact', 'decision', 'task', 'deadline'
    entity_id = Column(UUID(as_uuid=True), nullable=False)
    review_type = Column(String, nullable=False)  # 'incorrect', 'partial'
    rejection_reason = Column(
        String
    )  # 'Wrong Type', 'Wrong Value', 'Incomplete', 'Hallucinated', 'Duplicate'
