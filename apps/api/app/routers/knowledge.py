...
from app.models.core import User, Fact, Decision, Task, Deadline, KnowledgeReview, ExtractionFeedback
...
async def list_facts(
    ...
    stmt = select(Fact).where(Fact.user_id == current_user.id, Fact.review_state != "rejected")
    if context_id:
...
async def list_decisions(
    ...
    stmt = select(Decision).where(Decision.user_id == current_user.id, Decision.review_state != "rejected")
    if context_id:
...
async def list_tasks(
    ...
    stmt = select(Task).where(Task.user_id == current_user.id, Task.review_state != "rejected")
    if context_id:
...
async def list_deadlines(
    ...
    stmt = select(Deadline).where(Deadline.user_id == current_user.id, Deadline.review_state != "rejected")
    if context_id:
...
async def approve_decision(
    ...
    stmt = select(Decision).where(Decision.id == id, Decision.user_id == current_user.id)
    res = await db.execute(stmt)
    item = res.scalar_one_or_none()
    if not item:
...
async def approve_deadline(
    ...
    stmt = select(Deadline).where(Deadline.id == id, Deadline.user_id == current_user.id)
    res = await db.execute(stmt)
    item = res.scalar_one_or_none()
    if not item:
...
async def update_decision(
    ...
    edited = False
    if payload.chosen_option is not None and payload.chosen_option != item.chosen_option:
