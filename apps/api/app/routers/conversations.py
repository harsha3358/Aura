    ...
    for conv in conversations:
        ...
        for msg in messages:
            ...
            deadlines_stmt = select(Deadline).where(
                Deadline.source_message_id == msg.id
            )
...
async def create_conversation_message(
    ...
    message = Message(conversation_id=id, role=payload.role, content=payload.content)
    db.add(message)
    await db.commit()
    ...
    extraction_result = ExtractionResult(
        ...
        metadata={
            "confidence": 1.0,
            "reasoning": "Extraction skipped for non-user roles.",
        },
    )
    ...
    if payload.role == "user":
        ...
        if shift_detected and classification.get("new_context"):
            ...
            matched_ctx = next(
                (
                    ...
                    if c.name.lower().strip() == matched_name
                ),
                None,
            )
            if matched_ctx:
        ...
        extraction_result, observations = await extractor.extract_from_message(
            payload.content
        )
...
@router.post(
    "/messages", response_model=MessagePostResponse, status_code=status.HTTP_201_CREATED
)
