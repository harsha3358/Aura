...
class BriefingService:
    ...
    async def generate_brief(
        self, user: User, target_date: Optional[date] = None
    ) -> ExecutiveBrief:
        if target_date is None:
            target_date = datetime.utcnow().date()

        # 1. Gather all active contexts
        ctx_stmt = select(Context).where(Context.user_id == user.id, Context.is_active)
        ...
        structured = {
            ...
            "upcoming_deadlines": [
                f"{d.title} (due {d.due_at.strftime('%Y-%m-%d')})" for d in deadlines
            ],
        ...
        if not structured["top_priorities"]:
            structured["top_priorities"].append(
                "No immediate priorities. Focus on planning."
            )

        # Populate suggested next actions
        if tasks:
            structured["suggested_next_actions"].append(
                f"Begin working on: {tasks[0].task}"
            )
        if deadlines:
            structured["suggested_next_actions"].append(
                f"Prepare for upcoming deadline: {deadlines[0].title}"
            )
        if projects and not tasks:
            structured["suggested_next_actions"].append(
                f"Define first actionable tasks for project: {projects[0].project_name}"
            )

        if not structured["suggested_next_actions"]:
            structured["suggested_next_actions"].append(
                "Create a new project or start a session to capture work."
            )

        # 7. Render Brief to Text
        display_name = user.display_name or "Builder"
        lines = [f"GOOD MORNING {display_name.upper()}", ""]
