...
async def test_briefing_generation_and_retrieval(anyio_backend):
    ...
    await db.execute(
        delete(ExecutiveBrief).where(ExecutiveBrief.user_id == MOCK_USER_ID)
    )
    await db.execute(
        delete(ExtractionFeedback).where(ExtractionFeedback.user_id == MOCK_USER_ID)
    )
    await db.execute(
        delete(KnowledgeReview).where(KnowledgeReview.reviewer_id == MOCK_USER_ID)
    )
    ...
    if convo_ids:
        ...
        if msg_ids:
            await db.execute(
                delete(ContextAssignment).where(
                    (ContextAssignment.entity_type == "message")
                    & ContextAssignment.entity_id.in_(msg_ids)
                )
            )
    ...
    if ctx_ids:
        await db.execute(
            delete(ContextAssignment).where(ContextAssignment.context_id.in_(ctx_ids))
        )
