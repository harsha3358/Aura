...
async def test_add_message():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        response = await ac.post(
            "/api/v1/conversations", json={"title": "Message Chat"}
        )
        conv_id = response.json()["id"]

        response = await ac.post(
            f"/api/v1/conversations/{conv_id}/messages",
            json={"content": "Hello AURA", "role": "user"},
        )
        assert response.status_code == 201
...
async def test_submit_feedback():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        response = await ac.post(
            "/api/v1/conversations", json={"title": "Feedback Chat"}
        )
        conv_id = response.json()["id"]

        msg_resp = await ac.post(
            f"/api/v1/conversations/{conv_id}/messages",
            json={"content": "AURA feedback test message", "role": "user"},
        )
        msg_id = msg_resp.json()["message"]["id"]

        fb_resp = await ac.post(
            "/api/v1/extraction/feedback",
            json={
        ...
        return ExtractionResult(
            facts=[
                FactItem(
                    ...
                    entity="project",
                    confidence=0.95,
                    category=FactEnum.PROJECT_DETAIL,
                )
            ],
            decisions=[
                DecisionItem(
                    ...
                    entity="database",
                    confidence=0.90,
                    category=DecisionEnum.TECHNOLOGY,
                )
            ],
            considered_options=[],
            tasks=[
            ...
            deadlines=[
                DeadlineItem(
                    ...
                    entity="milestone",
                    confidence=0.80,
                    category=DeadlineEnum.MILESTONE,
                )
            ],
            contexts=[],
...
async def test_knowledge_review_workflow():
    app.dependency_overrides[get_extraction_service] = lambda: (
        PopulatedMockExtractionService()
    )

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        conv_resp = await ac.post(
            "/api/v1/conversations", json={"title": "Review Chat"}
        )
        ...
        rej_resp = await ac.post(
            ...
            json={"reason": "Wrong Type", "message_id": res_data["message"]["id"]},
        )
...
async def test_feedback_detailed_workflows():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        conv_resp = await ac.post(
            "/api/v1/conversations", json={"title": "Feedback Detailed"}
        )
        conv_id = conv_resp.json()["id"]
        msg_resp = await ac.post(
            f"/api/v1/conversations/{conv_id}/messages",
            json={"content": "A message to gather feedback", "role": "user"},
        )
        msg_id = msg_resp.json()["message"]["id"]
        ...
        fb_resp = await ac.post(
            "/api/v1/extraction/feedback", json={"feedback_type": "correct"}
        )
        ...
        fb_resp = await ac.post(
            "/api/v1/extraction/feedback",
            json={"extraction_run_id": "not-a-uuid", "feedback_type": "correct"},
        )
        assert fb_resp.status_code == 422  # validation error
        ...
        fb_resp = await ac.post(
            "/api/v1/extraction/feedback",
            json={"extraction_run_id": str(uuid.uuid4()), "feedback_type": "correct"},
        )
