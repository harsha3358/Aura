...
export const CaptureSession: React.FC = () => {
    ...
    switch (type) {
      case "fact":
        return {
          ...
          badge:
            "bg-[rgba(167,139,250,0.15)] border-[#a78bfa]/30 text-[#a78bfa]",
          label: "FACT",
        };
      case "decision":
        return {
          ...
          badge:
            "bg-[rgba(59,130,246,0.15)] border-[#60a5fa]/30 text-[#60a5fa]",
          label: "DECISION",
        };
      case "task":
        return {
          ...
          badge:
            "bg-[rgba(249,115,22,0.15)] border-[#fb923c]/30 text-[#fb923c]",
          label: "TASK",
        };
      case "deadline":
        return {
    ...
    return (
      ...
      >
        {isEditing ? (
          ...
          >
            <span
              className={`px-2 py-0.5 rounded text-[10px] font-bold tracking-wider shrink-0 ${style.badge}`}
            >
              EDITING
            </span>
            <input
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (type === "fact") payload.value = editingItemText;
                  ...
                  payload.chosen_option = editingItemText;
          ...
          <>
            <div className="flex items-center gap-1.5 min-w-0 max-w-full">
              ...
              <span
                className={`truncate max-w-[280px] md:max-w-[400px] text-[#e4e4e7] ${item.review_state === "rejected" ? "line-through opacity-50" : ""}`}
              >
                {getPrimaryText()}
              </span>
  ...
  return (
    <div className="flex h-full capture-modal" data-testid="capture-session">
      {/* Sidebar */}
      <aside
        className="w-64 border-r border-[#1e1e24] p-4 overflow-y-auto"
      ...
      >
        {activeConversation ? (
          <div className="flex-1 space-y-3 mb-4">
            {activeConversation.messages.map((msg: any, idx: number) => (
              ...
              >
                ...
                {msg.role === "user" && (
                  <div
                    className="flex flex-wrap gap-2 justify-start mt-1 max-w-full"
                    data-testid="extraction-chips"
                  >
                    {msg.facts?.map((item: any) =>
                      renderReviewChip("fact", item, msg.id),
                    )}
                    {msg.decisions?.map((item: any) =>
                      renderReviewChip("decision", item, msg.id),
                    )}
                    {msg.tasks?.map((item: any) =>
                      renderReviewChip("task", item, msg.id),
                    )}
                    {msg.deadlines?.map((item: any) =>
                      renderReviewChip("deadline", item, msg.id),
                    )}
                  </div>
                )}
              </div>
          ...
          <div
            className="flex-1 flex items-center justify-center text-[#71717a]"
